...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  ...
  int curpage=1;//当前页
				...
				String fysql="select count(id) as ss from yingpinjilu where yingpinren='"+request.getSession().getAttribute("username")+"'";
  ...
  zys=zgs/page_record+1;
   ...
   sql="";
  sql="select top "+page_record+" * from yingpinjilu where yingpinren='"+request.getSession().getAttribute("username")+"' and id not in (select top "+((curpage-1)*page_record)+" id from yingpinjilu where yingpinren='"+request.getSession().getAttribute("username")+"' order by id desc) ";
...
%>
  <tr>
    ...
    <td><%=gongsibianhao %></td><td><%=gongsimingcheng %></td><td><%=gangweimingcheng %></td><td><%=xinzidaiyu %></td><td><%=yingpinren %></td><td><%=gongsihuifu %></td><td width='80' align='center'><%=issh%></td>
    <td width="138" align="center"><%=addtime %></td>
    <td width="60" align="center"><a href="yingpinjilu_updt.jsp?id=<%=id%>"></a>  <a href="del.jsp?id=<%=id %>&tablename=yingpinjilu" onClick="return confirm('真的要删除？')">删除</a></td>
  </tr>
  	<%
  }
   %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="yingpinjilu_list2.jsp?page=1">首页</a>　<a href="yingpinjilu_list2.jsp?page=<%= curpage-1%>">上一页</a>　<A href="yingpinjilu_list2.jsp?page=<%= curpage+1%>">下一页</A>　<a href="yingpinjilu_list2.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
