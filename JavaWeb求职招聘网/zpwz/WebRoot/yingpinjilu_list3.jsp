...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td bgcolor='#CCFFFF'>公司编号</td><td bgcolor='#CCFFFF'>公司名称</td><td bgcolor='#CCFFFF'>岗位名称</td><td bgcolor='#CCFFFF'>薪资待遇</td><td bgcolor='#CCFFFF'>应聘人</td><td bgcolor='#CCFFFF'>公司回复</td>
    <td width='80' align='center' bgcolor='#CCFFFF'>应聘成功</td>
  ...
  int curpage=1;//当前页
				...
				String fysql="select count(id) as ss from yingpinjilu where gongsibianhao='"+request.getSession().getAttribute("username")+"'";
  ...
  zys=zgs/page_record+1;
   ...
   sql="";
  sql="select top "+page_record+" * from yingpinjilu where gongsibianhao='"+request.getSession().getAttribute("username")+"' and id not in (select top "+((curpage-1)*page_record)+" id from yingpinjilu where gongsibianhao='"+request.getSession().getAttribute("username")+"' order by id desc) ";
...
%>
  <tr>
    ...
    <td><%=gongsibianhao %></td><td><%=gongsimingcheng %></td><td><%=gangweimingcheng %></td><td><%=xinzidaiyu %></td><td><%=yingpinren %></td><td><%=gongsihuifu %></td>
    <td width='80' align='center'><a href="sh.jsp?id=<%=id%>&yuan=<%=issh%>&tablename=yingpinjilu" onClick="return confirm('您确定要执行此操作？')"><%=issh%></a></td>
    <td width="138" align="center"><%=addtime %></td>
    <td width="60" align="center"><a href="yingpinjilu_updtlb.jsp?id=<%=id%>">回复</a>  <a href="del.jsp?id=<%=id %>&tablename=yingpinjilu" onClick="return confirm('真的要删除？')">删除</a></td>
  </tr>
  	<%
  }
   %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="yingpinjilu_list3.jsp?page=1">首页</a>　<a href="yingpinjilu_list3.jsp?page=<%= curpage-1%>">上一页</a>　<A href="yingpinjilu_list3.jsp?page=<%= curpage+1%>">下一页</A>　<a href="yingpinjilu_list3.jsp?page=<%=zys %>">尾页</A>　当前第<FONT color=red><%=curpage %></FONT>页/共<FONT color=red><%=zys %></FONT>页</p>
  </body>
</html>
