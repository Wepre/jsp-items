 ...
 %>
  ...
  <form name="form1" id="form1" method="post" action="">
   ...
   <select name="leibie" id="leibie">
	 ...
	 <option value="变幻图">变幻图</option>
   </select> 
   添加人：
   <input name="tianjiaren" type="text" id="tianjiaren" />
   <input type="submit" name="Submit" value="查找" />
   <input type="button" name="Submit2" value="新增" onClick="javascript:location.href='xinwentongzhi_add2.jsp';" />
  </form>
  ...
  <form name="form2" method="post" action="delall.jsp">
    ...
    <% 
	new CommDAO().delete(request,"xinwentongzhi"); 
    String url = "xinwentongzhi_list3.jsp?1=1"; 
    String sql =  "select * from xinwentongzhi where leibie='变幻图' ";
	...
	i++;
     %>
      <tr>
		...
		{
		%>
            <a href='<%=map.get("shouyetupian") %>' target='_blank'><img src='<%=map.get("shouyetupian") %>' width=389 height=99 border=0 /></a>
            <%
		}
	%>
        </td>
